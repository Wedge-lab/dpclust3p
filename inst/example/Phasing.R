########################################
#Comments from Haoqi
#For Dream bam file, first to construct index through samtools
#two fai.fa, one is in format chrX, the other is simple figure without chr
#demo from DPC3p
...
if (length(args) > 4) {
  ...
  fai_file = "/gpfs2/well/wedge/kyo641/RealSample_EGA/PhasingOutput/hg19.new.fa.fai"
  ign_file = "/gpfs2/well/wedge/kyo641/RealSample_EGA/PhasingOutput/ignore_dpclust_preprocessing.txt"
}
...
mut_mut_phasing = function(loci_file, phased_file, bam_file, bai_file, max_distance, minMapQ=20) {
  ...
  print("START")
  muts <- read.delim(loci_file, header=T, row.names=NULL, stringsAsFactors=F)
...
mut_mut_phasing(loci_file="/gpfs2/well/wedge/kyo641/RealSample_EGA/A22_Phasing/A22_loci.txt",
