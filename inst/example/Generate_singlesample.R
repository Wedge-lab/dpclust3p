...
ce= as.data.frame(read.table(paste0(batt_path, samplenames[i,samplename], "_rho_and_psi.txt"),header=TRUE,sep='\t'))
cellularity = as.vector(as.numeric(ce[1,"rho"]))

subclone.files = as.vector(paste0(batt_path, samplenames[i,samplename], "_subclones.txt"))

INFO= as.data.frame(read.table(paste0(ac_dir, samplename, "_ac_match.txt"),header=TRUE,sep='\t'))[,1:2]
info= INFO

colnames(INFO) = c("chr", "pos")

mu=read.table(paste0(ac_dir, samplename, "_mutCount.txt"), sep='\t',header=TRUE,stringsAsFactors=F)

w=read.table(paste0(ac_dir,  samplename, "_WTCount.txt"), sep='\t',header=TRUE,stringsAsFactors=F)
...
if(is.null(out.files)){
  out.files = cbind(paste(samplename,"_ssDPI.txt",sep=""),paste(samplename,"_ssDPI_WCL.txt",sep=""))
}
